module TradeCommands
  def send_offer(mine, they, link, message = '')
    raise "No account logged in, #{self}" unless @logged_in

    partner_steam_id, token = verify_profileid_or_trade_link_or_steamid(link)
    if token == nil
      verdict = verify_friendship(partner_steam_id)
      persona = verdict["accountname"]
      if verdict["success"] == false
        raise "#{partner_steam_id} (#{persona}) is not in your friendlist, a trade link is required to send an offer to this account"
      end
    end
    ...
    if token != nil
      ...
      # there is no token cause steamid given
    ...
    if response["needs_mobile_confirmation"] == true
      if @identity_secret != nil && @steam_id != nil
        sleep(0.6)
  ...
  def sell_items(items, price)
    raise "no account logged in, #{self} " if @logged_in == false
    ...
    i = 0
    items.each { |asset|
      asset['sessionid'] = session_id_cookie()
      asset['price'] = price
      verd = { 'sucess' => false }
      tries = 0
